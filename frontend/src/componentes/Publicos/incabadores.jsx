...
const Incabador = () => {
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/incubadoras');
  ...
  const copiarAlPortapapeles = async (texto) => {
    try {
      ...
      alert('Clave copiada al portapapeles'); // Alerta para confirmar al usuario
    } catch (err) {
      ...
      <div className="catalogo-container">
        {productosActuales.map((producto, index) => (
          <div className="producto" key={index}>
            ...
            <button id='b' className="ver-mas-button" onClick={() => openModal(producto)}>Ver más...</button>
          </div>
        ))}
      </div>
      ...
      {isModalOpen && (
        <div className="modal" onClick={closeModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="producto-header">
              <h3>{selectedProductDetails.nombre}</h3>
            </div>
            <div className="modal-body">
              ...
              <div className="modal-descripcion">
                ...
                <div>
                  <div className="modal-detalle-titulo">◈ dimensión</div>
                  <p>{selectedProductDetails.dimension}</p>
                </div>
                <div className="modal-detalle-titulo" onClick={() => copiarAlPortapapeles(selectedProductDetails.Clave)}>
                  ◈ Clave
                </div>
                <p>{selectedProductDetails.Clave}</p>
              </div>
            </div>
          </div>
        </div>
